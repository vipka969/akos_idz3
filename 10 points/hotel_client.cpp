...
void send_to_observers(MessageType msg_type, Gender gender, int client_id, const std::string& details = "") {
    ...
    for (int observer_id = 1; observer_id <= 5; observer_id++) {
        std::stringstream fifo_name;
        fifo_name << FIFO_BASE_NAME << observer_id;
...
void client_process(int client_id) {
    ...
    if (shm_fd == -1 || data == MAP_FAILED) {
        std::cout << "Клиент " << client_id << ": Гостиница не найдена!" << std::endl;
        return;
    }
    ...
    if (sem_admin == SEM_FAILED || sem_output == SEM_FAILED) {
    ...
    sem_wait(sem_admin);
    bool hotel_active = data->hotel_active;
    sem_post(sem_admin);
    
    if (!hotel_active || client_stop) {
        std::cout << "Клиент " << client_id << ": Гостиница закрыта" << std::endl;
        cleanup_clients(sem_admin, sem_output, data, shm_fd);
        return;
    }
    ...
    send_to_observers(MSG_CLIENT_ARRIVED, gender, client_id);
    ...
    if (!data->hotel_active || client_stop) {
        sem_post(sem_admin);
        std::cout << "Клиент " << client_id << ": Гостиница закрыта" << std::endl;
        send_to_observers(MSG_CLIENT_REJECTED, gender, client_id, "Гостиница закрыта");
        cleanup_clients(sem_admin, sem_output, data, shm_fd);
        return;
    }
    ...
    // Сначала пробуем одноместный номер
    if (data->single_rooms > 0) {
        ...
        sem_wait(sem_output);
        std::cout << "Клиент " << client_id << " (" << gender_str << "): получил одноместный номер" << std::endl;
        sem_post(sem_output);
        
        send_to_observers(MSG_CLIENT_SERVED, gender, client_id, "одноместный номер");
        
        int stay_time = 12 + (rand() % 9);
        std::cout << "Клиент " << client_id << " ночует " << stay_time << " секунд" << std::endl;
        sleep(stay_time);
    }
    // Пробуем двухместный номер с ожидающим
    else if (data->double_rooms > 0) {
        // Проверяем есть ли ожидающий того же пола
        ...
        if (found_waiting) {
            // Нашли пару - занимаем двухместный номер
            data->double_rooms--;
            got_room = true;
            ...
            send_to_observers(MSG_CLIENT_SERVED, gender, client_id, "двухместный номер с соседом");
            
            int stay_time = 15 + (rand() % 11);
            std::cout << "Клиент " << client_id << " ночует " << stay_time << " секунд" << std::endl;
            sleep(stay_time);
        }
        else {
            // Не нашли пару - встаем в очередь
            bool added = false;
            if (gender == LADY) {
            ...
            if (!added) {
                ...
                send_to_observers(MSG_CLIENT_REJECTED, gender, client_id, "очередь переполнена");
                cleanup_clients(sem_admin, sem_output, data, shm_fd);
                return;
            }
            ...
            send_to_observers(MSG_CLIENT_ARRIVED, gender, client_id, "ждет соседа для двухместного номера");
            
            bool found_pair = false;
            for (int i = 0; i < 15 && !found_pair && !client_stop; i++) {
                sleep(1);
            ...
            if (found_pair && !client_stop) {
                sem_wait(sem_output);
                std::cout << "Клиент " << client_id << " (" << gender_str << "): заселился в двухместный номер с соседом" << std::endl;
                sem_post(sem_output);
                
                send_to_observers(MSG_CLIENT_SERVED, gender, client_id, "двухместный номер (дождался соседа)");
                ...
                send_to_observers(MSG_CLIENT_REJECTED, gender, client_id, "не дождался соседа");
                cleanup_clients(sem_admin, sem_output, data, shm_fd);
                return;
            }
        }
    ...
    // Нет свободных номеров
        ...
        send_to_observers(MSG_CLIENT_REJECTED, gender, client_id, "нет свободных номеров");
        cleanup_clients(sem_admin, sem_output, data, shm_fd);
        return;
    }
    ...
    if (got_room && !client_stop) {
        ...
        send_to_observers(MSG_CLIENT_DEPARTED, gender, client_id, "освобождает " + room_type + " номер");
    }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Информация отправляется всем наблюдателям" << std::endl;
